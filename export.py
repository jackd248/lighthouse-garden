...
def export_html ():
    ...
    results = add_average_data_and_sort(generate.get_results())
    for result in results:
        html.write('<tr>')
        html.write('<td><strong>' + result['title'] + '</strong><br/><a href="' + result['url'] + '" target="_blank">' + result['url'] + '</a></td>')
        html.write('<td><a href="' + result['report'] + '" target="_blank">' + generate_percentage_circle(int(round(result['average']))) + '</a></td>')
...
def add_average_data_and_sort(results):
    for result in results:
        result['average'] = get_average_by_attribute(generate.get_target_by_attribute(result['title'],'title'),'performance')
